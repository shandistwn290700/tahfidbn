...
const Row: FC<{ member: RankedStudent; showClassRank: boolean }> = ({
  member,
  showClassRank,
}) => {
  const rankClass =
        ...
        <div class="flex items-center gap-3">
          <div class="flex-1 rounded-full h-2 bg-slate-100 dark:bg-slate-800">
            <div
              ...
              style={`width: ${member.total_memorized > 0 ? Math.max(member.progress_percent, 1.5) : 0}%`}
            />
          </div>
...
export const LeaderboardTable: FC<{
  ...
  const buildHref = (targetPage: number) => {
    ...
    if (sort && sort !== "juz") params.set("urut", sort);
  ...
  return (
    <div class={`${CARD} w-full overflow-hidden mb-8`}>
      <div class="hidden md:grid grid-cols-12 gap-4 px-6 py-4 border-b border-border-light dark:border-border-light-dark bg-slate-50/50 dark:bg-slate-800/50 text-text-secondary dark:text-text-secondary-dark text-xs font-bold uppercase tracking-wider">
        ...
        <div class="col-span-3">Posisi Hafalan</div>
          ...
          <EmptyState
            ...
            description={
              search || classId !== null
                ...
                : "Peringkat akan terisi setelah guru mulai menginput hafalan siswa."
            }
          />
        ) : (
          members.map((member) => (
            <Row member={member} showClassRank={classId !== null} />
          ))
        )}
      </div>
